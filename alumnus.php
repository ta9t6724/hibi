<?php  
    ...
    $sql = "SELECT `u`.`name`,`u`.`graduation_date`,`f`.`user_id`,`f`.`picture`,`f`.`created` FROM `users` `u` LEFT JOIN (SELECT `f`.`user_id`,`f`.`picture`,`f`.`created` FROM `feeds` `f` GROUP BY `f`.`user_id` ORDER BY `created`) AS `f` ON `u`.`id` = `f`.`user_id` WHERE `graduation_date` < CURRENT_DATE() LIMIT $start, $page_row_number";
...
<html lang="ja">
  <head>
    ...
    <link href="assets/css/hibi.css" rel="stylesheet">
  ...
  <body>
       ...
       <?php include("navbar.php"); ?>
        ...
        <div class="col-md-10 main-content">
          ...
          <div class="row">
            ...
            <div class="col-md-3">
              <a href="my_page.php?user_id=<?php echo $alumnu["user_id"]; ?>"><img src="assets/img/<?php echo $alumnu["picture"]; ?>" class="hibi_pic" style="width: 250px; height: auto; margin: 60px 0 20px 0"></a>
          ...
          <div aria-label="Page navigation">
           <ul class="pager">
             ...
             <?php }else{ ?>
                <li class="previous"><a href="alumnus.php?page=<?php echo $page-1; ?>"><span aria-hidden="true">&larr;</span>前へ</a></li>
             <?php } ?>
              <?php if ($page == $all_page_number){ ?>
                <li class="next disabled"><a href="#">次へ<span aria-hidden="true">&rarr;</span></a></li>
              <?php }else{ ?>
                <li class="next"><a href="alumnus.php?page=<?php echo $page+1; ?>">次へ<span aria-hidden="true">&rarr;</span></a></li>
              <?php } ?>
            </ul>
          </div>
        </div>
    ...
    <!-- Optional JavaScript -->
    <!-- jQuery f   irst, then Popper.js, then Bootstrap JS -->
